...
class GRNN:
    ...
    def __init__(self, X_train, y_train, X_test, y_test, sigma, activation='euclidean', p_minkowski=2):
        self.sigma = sigma
    ...
    def output(self, i):
        ...
        elif self.activation == "hamming":
            distances = np.sum(self.X_test[i] != self.X_train, axis=1)
        elif self.activation == "mahalanobis":
            X_test = self.X_test[i]
